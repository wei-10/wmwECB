...
class ImageList(Dataset):
    def __init__(self, data_root, data_path, transform_w=None, transform_str=None, sample_masks=None, use_cgct_mask=False):
        ...
        if self.use_cgct_mask:
            self.sample_masks = (
                sample_masks if sample_masks is not None else torch.zeros(len(self.imgs)).float()
            )
        else:
...
def build_data(config, debug=True):
    ...
    if method == "SSDA":
        ...
        image_set_file_s = os.path.join(data_label, f'labeled_source_images_{source_name}.txt')
        image_set_file_s_test = os.path.join(data_label, f'validation_target_images_{source_name}_3.txt')

        image_set_file_tl = os.path.join(data_label, f'labeled_target_images_{target_name}_{num}.txt')
        image_set_file_tu = image_set_file_tu_test = os.path.join(data_label, f'unlabeled_target_images_{target_name}_{num}.txt')
        image_set_file_tu_val = os.path.join(data_label, f'validation_target_images_{target_name}_3.txt')
        ...
        image_set_file_s = os.path.join(data_label, f'{source_name}{prefix["train"]}')
        image_set_file_s_test = os.path.join(data_label, f'{source_name}{prefix["test"]}')

        image_set_file_tu = os.path.join(data_label, f'{target_name}{prefix["train"]}')
        image_set_file_tu_test = os.path.join(data_label, f'{target_name}{prefix["test"]}')
